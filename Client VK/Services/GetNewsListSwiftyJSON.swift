...
struct NewsResponseItemSwifty {
    ...
    init(json: JSON){
        ...
        if json["attachments"][0]["type"] == "link" {
            self.imgUrl = json["attachments"][0]["link"]["photo"]["sizes"][0]["url"].stringValue
        }
...
//    func parse(_ data: Data?) -> [NewsResponseItemSwifty] {
    ...
    func makeNewsList(_ items: [NewsResponseItemSwifty],
        ...
        for item in items {
            ...
            var newItem = News(name: "", avatar: "", date: stringDate, textNews: item.text, imageNews: item.imgUrl, likes: item.likes, comments: item.comments, reposts: item.reposts, views: item.views)
